...
Little’s Irregularity Index (LII) ในแบบจำลองเรขาคณิต 2D ของโครงงานนี้ กำหนดโครงสร้างเป็น **Polyline Path Length Model** ($LII = \sum_{i=1}^5 d(P_i, P_{i+1})$) ซึ่งเป็นผลรวมของระยะทางยูคลิดระหว่างพิกัดจุดเรียงลำดับ 6 จุดบนขอบฟัน (incisal edge) รวม 5 ช่วง
...
- ใช้แทนทันตแพทย์ได้
- ค่า LII ในแอปพลิเคชันนี้คือ Polyline Path Length ตัวแบบ 2D ไม่ใช่ Clinical Displacement Index แบบดั้งเดิม
